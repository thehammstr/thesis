...
This chapter will describe how the iceberg profiling problem was posed in the framework of GraphSLAM in order to allow the reader to reproduce the mapping results presented in Chapter \ref{ch.Results}. To provide context, it will give a brief overview of some key components of the method. However, it will not go into the low-level details of how the GraphSLAM algorithm implements its optimization. For more information, refer to the sources cited in Section \ref{sec.SLAM}.

\section{GraphSLAM overview}

This section presents the key components of the GraphSLAM algorithm to provide sufficient context for understanding its application to iceberg profiling. It introduces the state $x_t$, input $u_t$, motion model $g()$, and measurement model $h()$, and defines how they all interact to produce a result. The following section will then detail how each of these is defined for application to iceberg profiling.
...
Once the graph has been built, it can be optimized via a generic nonlinear least squares approach. To see this, Equations \ref{eq:Gauss1} and \ref{eq:Gauss2} are expanded by the definition of the Normal distribution.
...
This section gives details on how all the components of iceberg profiling as described in Chapter \ref{ch.ProblemStatement} can be combined within the GraphSLAM framework to  yield an accurate reconstruction. 
